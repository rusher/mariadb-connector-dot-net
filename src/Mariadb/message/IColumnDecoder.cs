  ...
  static IColumnDecoder decode(IReadableByteBuf buf, bool extendedInfo) {
    ...
    if (extendedInfo) {
      ...
      if (buf.GetByte() != 0) {
        ...
        IReadableByteBuf subPacket = buf.ReadLengthBuffer();
    ...
    DataType dataType = (DataType) buf.ReadUnsignedByte();
    int flags = buf.ReadUnsignedShort();
    byte decimals = buf.ReadByte();
    
    Func<IReadableByteBuf,int,long,DataType ,byte,int,int[] ,string ,string , IColumnDecoder> constructor =
