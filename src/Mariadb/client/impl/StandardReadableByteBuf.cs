...
public class StandardReadableByteBuf : IReadableByteBuf
{
  ...
  public long Atoll(int length) {
    ...
    if (length > 0 && _buf[Pos] == 45) { // minus sign
      negate = true;
      Pos++;
      idx++;
    }
  ...
  public int ReadMedium() {
    ...
    if ((value & 0x800000) != 0) {
      return unchecked((int) (value | 0xff000000));
    }
    return value;
  }
  ...
  public byte[] ReadBytesNullEnd() {
    ...
    int cnt = 0;
    while (ReadableBytes() > 0 && (_buf[Pos++] != 0)) {
      cnt++;
    }
  ...
  public float ReadFloat() {
    float f = BitConverter.ToSingle(_buf, Pos);
    Pos += 4;
    return f;
  }

  public double ReadDouble() {
    double d = BitConverter.ToDouble(_buf, Pos);
    Pos += 8;
    return d;
  }
